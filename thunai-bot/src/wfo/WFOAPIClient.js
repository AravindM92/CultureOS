...
class WFOAPIClient {
     ...
     */
    async checkWFODataNeeded(userId, collectionType = 'weekly') {
        try {
            const response = await this.client.get(`/api/v1/availability/check/${userId}?collection_type=${collectionType}`);
