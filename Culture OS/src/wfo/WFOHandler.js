...
class WFOHandler {
    ...
    async canHandle(message, userContext = {}) {
        ...
        // SIMPLE: Only trigger on "Week" keyword - no complications!
    ...
    async startWFOConversation(message, userId, userContext) {
        try {
            ...
            const collectionCheck = await this.wfoAPI.checkWFODataNeeded(userId);
    ...
    async handleConfirmation(message, userId, userState) {
        ...
        if (isConfirmed) {
            ...
            await this.wfoAPI.saveWFOSchedule(userId, userState.extractedData, 'user_confirmed');
